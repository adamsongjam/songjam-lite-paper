...
\section{Intellectual Property and Sovereignty}
...
Though such acts may protect commercial interests, they do not protect the natural rights of an individual to their voice, and the ability to control how it is used.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        node distance=2cm,
        owner/.style={circle, draw, fill=blue!20, minimum size=1.8cm, text width=2.2cm, align=center},
        sbt/.style={rectangle, draw, fill=green!20, minimum width=3cm, minimum height=1.2cm, text width=2.8cm, align=center},
        ...
        \node[above=0.5cm] at (-4,0) {\textbf{Self-Custody}};
        \node[owner] (owner1) at (-4,0) {Voice\\Owner};
        \node[sbt] (sbt1) at (0,0) {Proto-SBT\\\small{Encryption}};
        \node[tee] (tee1) at (4,0) {TEE Network\\\small{Key Derivation}};
        \node[ipfs] (ipfs1) at (0,-3) {IPFS\\\small{Storage}};
        
        % Connections for self-custody
        \draw[arrow] (owner1) -- node[above] {Controls} (sbt1);
        \draw[arrow] (sbt1) -- node[above] {Derives} (tee1);
        \draw[arrow] (sbt1) -- node[right] {Manages} (ipfs1);
        
        % Custodial diagram
        \node[above=0.5cm] at (12,0) {\textbf{Custodial}};
        \node[owner] (owner2) at (8,0) {Voice\\Owner};
        \node[sbt] (sbt2) at (12,0) {Proto-SBT\\\small{Encryption}};
        \node[tee] (tee2) at (16,0) {TEE Network\\\small{Key Derivation}};
        \node[ipfs] (ipfs2) at (12,-3) {IPFS\\\small{Storage}};
        \node[rectangle, draw, fill=red!20, minimum width=4cm, minimum height=2.5cm, text width=3.8cm, align=center] (custodian) at (12,-1.5) {Data Custodian\\\small{Nation State}};
        
        % Connections for custodial
        \draw[arrow] (owner2) -- node[above] {Delegates} (sbt2);
        \draw[arrow] (sbt2) -- node[above] {Derives} (tee2);
        \draw[arrow] (sbt2) -- node[right] {Manages} (ipfs2);
    \end{tikzpicture}
    \caption{Comparison of self-custody and custodial solutions for voice data management}
    \label{fig:custody-comparison}
\end{figure}
\vspace{0.1cm}
